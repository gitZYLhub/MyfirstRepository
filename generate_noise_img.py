...
def generateSingleSinusoid(img_size, angle, phase):
    ...
    sinusoid = (sinepath*np.sin(angle) + sinepath.T*np.cos(angle)) * 2 * np.pi # number of cycles
...
for i in range(num_loops):
    patches = generatePatches(img_size, nscale=5)
    noise, params, store_amplitude, noise_scales = generateNoise(img_size, patches, nscale=5)
